...
class SenderNodeCan0 : public rclcpp::Node {
public:
    SenderNodeCan0(const std::string& ifname = "can0") : Node("Sender_node_can0"), ifname_(ifname) {
        
        // kp, kd, ki, out_max, i_sum_max
        this->declare_parameter<std::vector<double>>("speed_gains_1_can0", {17.0, 200.0, 0.05, 40000, 1000.0});
        this->declare_parameter<std::vector<double>>("speed_gains_2_can0", {17.0, 200.0, 0.05, 40000, 1000.0});
        this->declare_parameter<std::vector<double>>("speed_gains_3_can0", {17.0, 200.0, 0.05, 40000, 1000.0});
        this->declare_parameter<std::vector<double>>("speed_gains_4_can0", {17.0, 200.0, 0.05, 40000, 1000.0});
        this->declare_parameter<std::vector<double>>("speed_gains_5_can0", {17.0, 200.0, 0.05, 40000, 1000.0});
        this->declare_parameter<std::vector<double>>("speed_gains_6_can0", {17.0, 200.0, 0.05, 40000, 1000.0});
        this->declare_parameter<std::vector<double>>("speed_gains_7_can0", {17.0, 200.0, 0.05, 40000, 1000.0});
        this->declare_parameter<std::vector<double>>("speed_gains_8_can0", {17.0, 200.0, 0.05, 40000, 1000.0});

        this->declare_parameter<std::vector<double>>("position_gains_1_can0", {3000.0, 0.0, 0.5, 2000.0, 1000.0});
        this->declare_parameter<std::vector<double>>("position_gains_2_can0", {3000.0, 0.0, 0.5, 2000.0, 1000.0});
        this->declare_parameter<std::vector<double>>("position_gains_3_can0", {3000.0, 0.0, 0.5, 2000.0, 1000.0});
        this->declare_parameter<std::vector<double>>("position_gains_4_can0", {3000.0, 0.0, 0.5, 2000.0, 1000.0});
        this->declare_parameter<std::vector<double>>("position_gains_5_can0", {3000.0, 0.0, 0.5, 2000.0, 1000.0});
        this->declare_parameter<std::vector<double>>("position_gains_6_can0", {3000.0, 0.0, 0.5, 2000.0, 1000.0});
        this->declare_parameter<std::vector<double>>("position_gains_7_can0", {3000.0, 0.0, 0.5, 2000.0, 1000.0});
        this->declare_parameter<std::vector<double>>("position_gains_8_can0", {3000.0, 0.0, 0.5, 2000.0, 1000.0});
        ...
        target_sub_ = this->create_subscription<robomas_package_2::msg::MotorCmdArray>("motor_cmd_array_can0", 10,
            std::bind(&SenderNodeCan0::target_callback, this, std::placeholders::_1));
        RCLCPP_INFO(this->get_logger(), "Subscribed to motor_cmd_array_can0 topic");
        can_sub_ = this->create_subscription<robomas_package_2::msg::CanFrame>("can_frame_tx_can0", 10,
            std::bind(&SenderNodeCan0::canframe_callback, this, std::placeholders::_1));
        RCLCPP_INFO(this->get_logger(), "Subscribed to can_frame_tx_can0 topic");
        ems_sub_ = this->create_subscription<robomas_package_2::msg::Ems>("ems_tx_can0", 10,
            std::bind(&SenderNodeCan0::ems_callback, this, std::placeholders::_1));
        RCLCPP_INFO(this->get_logger(), "Subscribed to ems_tx_can0 topic");
...
int main(int argc, char * argv[])
{
    ...
    auto node = std::make_shared<SenderNodeCan0>("can0");
    rclcpp::spin(node);
    rclcpp::shutdown();
    return 0;
}