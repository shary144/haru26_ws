...
shoot()<br>- [/target],<br>- [/robomas/cmd],<br>- [/robomas/feedback]|射出機構の速度を裏で調整して撃鉄がうまく一周して戻るようにする
grip()<br>- [/target]<br>- [robomas/cmd]|把持機構の起動
...
```cpp
#include "topicIO.hpp"
class AnyNode : topicIO::topicNode{
AnyNode(): topicIO::topicNode("any_node"){
...
class AnyNode : public rclcpp::Node, public ParamMixin<AnyNode> 
{
public:
    AnyNode(const rclcpp::NodeOptions& options): ParamMixin(options) {
        //一回だけ呼ばれるところに置くべき
        this->num = 0
        this->param("num",this->num);
        //yamlファイルを入れたなら動作前にパラメータに同期される
        //動作中でもコマンドラインから変更できる
    }
private:
    int64_t num;
...
### frame_vizについて
`AnyNode`がrvizでFrameを可視化するmarkerを発行する
`FrameViz<AnyNode>`を継承するとクラス内で
- viz_pos(Mytf2::Frame base_frame, My_tf2::Pose pos)
- viz_ball(ballchache::BallChache.ballarray)

が使えるようになる。
viz_posはmy_tf2を可視化して、viz_ballはballchacheを可視化する
